...
def obtener_longitud_palabra(turno,lista_palabras_ordenadas):
...
# Dario
def confirmar_palabra(palabra_a_confirmar,lista_palabras_ingresadas,lista_palabras_ordenadas):
    posicion= lista_palabras_ingresadas.index(palabra_a_confirmar)
    if palabra_a_confirmar in lista_palabras_ordenadas and palabra_a_confirmar == lista_palabras_ordenadas[posicion]:
        resultado = True
    else:
        resultado = False
    return resultado
...
# Sebas
def verificador_aciertos_errores(tupla_de_veripalabra, cant_aciertos, cant_errores,lista_palabras_ingresadas,lista_palabras_ordenadas):
...
def jugar(letras_participantes:list,palabras_del_juego:list,definiciones:dict):
    turno=0
    cant_aciertos= 0
    cant_errores=0
    quiere_jugar=True
    ...
    while turno <= len(lista_palabras_ordenadas):
        ...
        if turno >= len(lista_palabras_ordenadas):
            ...
            #if turno != len(lista_palabras_ordenadas):
            x=verificador_de_palabra(turno) #Cambiar variable X
            listar_palabras_de_usuario(x)
            verificador_aciertos_errores(x, cant_aciertos, cant_errores,palabras_i)
...
def main():
    ...
    #jugar(letras_del_juego,palabras_del_juego,definiciones)  #ITEM4 ETAPA 4
