...
def jugar(letras_participantes:list,palabras_del_juego:list,definiciones:dict,lista_palabras_ingresadas):
    turno=0
    cant_aciertos= 0
    cant_errores=0
    quiere_jugar=True

    while turno <= len(palabras_del_juego:list):
        print(mostrar_rosco_letras(letras_participantes))
        print(resultado_palabra(lista_palabras_ingresadas,letras_participantes:list,palabras_del_juego:list))
        print('Aciertos: '+ str(cant_aciertos))
        print('Errores: '+ str(cant_errores))
        if turno >= len(palabras_del_juego:list):
            print("funcion que rrecorra todo lo que escribiste y lo que deberia ser")
            print("puntaje final:" )
        else:
            print(turnos(letras_participantes[turno],turno,lista_palabras_ordenadas))
